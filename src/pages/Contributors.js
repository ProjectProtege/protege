      ...
      .then(
        (result) => {
          ...
          let simplifiedContributors = result.map(contributor => {
            const { url, id, avatar_url, login, html_url, contributions } = contributor
    ...
    return (
      <div className='flex flex-wrap justify-start'>
        {contributors.map((contributor) => {
          ...
          if(login === 'ImgBotApp'){return null}
          return (
          <div className='ml-2'>
            <IndividualContributor key={id} url={url} contributor={contributor} contributions={contributions}/>
          </div>)
        })}
      </div>
    );
...
const IndividualContributor = (props) => {
  ...
  if (error) {
    ...
    const { name, twitter_username, avatar_url, blog, html_url } = contributorData
    console.log(blog)
    return (
      <div className='flex m-4 p-4 mx-auto justify-start text-center bg-white shadow border-l-4 border-teal-500 transform hover:scale-105 hover:shadow-lg transition duration-150 ease-in-out'>
        <div className='flex flex-col ml-2 items-center justify-between'>
          <img className='rounded-full h-16 w-16 mx-auto' src={avatar_url} alt={`${name} avatar`} />
        </div>
        <div className='flex leading-none flex-col justify-around text-left mt-0 ml-4'>
          {name ? 
        ...
        {twitter_username && 
          ...
          className='flex items-center text-teal-300 hover:text-teal-600 transition-colors ease-in-out duration-75 w-auto'
        ...
        <a href={html_url} 
          className='ml-1 hover:text-teal-700' 
          target='_blank' rel="noopener noreferrer"
        >
          <svg
                className='fill-current flex-shrink w-4 hover:text-teal-500'
                aria-hidden='true'
                focusable='false'
                data-prefix='fab'
                data-icon='twitter'
