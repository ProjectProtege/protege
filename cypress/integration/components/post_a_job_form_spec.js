describe('post a job form', () => {
  ...
  it('tests inputs all the form fields', () => {
    cy.get('input[name="jobtitle"]')
      ...
      .should('value', 'Junior Developer')
    cy.get('select[name="roleFocus"]')
      .select('Front-end')
      .should('value', 'Front-end')
    cy.get('select[name="positionType"]')
      .select('Full-time')
      .should('value', 'Full-time')
    // Quill Input
    cy.get('.ql-editor').first().type('This is a GREAT gig.')
    cy.get('.ql-editor').first().children().contains('This is a GREAT gig.')
    cy.get('input[name="howToApply"]')
      .type('https://indeed.com?snakeholeLounge')
      .should('value', 'https://indeed.com?snakeholeLounge')
    cy.get('input[name="companyName"]')
      .type('Snakehole Lounge')
      .should('value', 'Snakehole Lounge')
    cy.get('input[name="companyWebsite"]')
      .type('https://parksandrecreation.fandom.com/wiki/The_Snakehole_Lounge')
      .should(
        'value',
        'https://parksandrecreation.fandom.com/wiki/The_Snakehole_Lounge'
      )
    cy.get('input[name="companyEmail"]')
      .type('tom@snakeholelounge.club')
      .should('value', 'tom@snakeholelounge.club')
    ...
    cy.fixture('SnakeholeLoungeLogo.png').then((fileContent) => {
      cy.get("[data-cy='company-logo-upload']").upload(
        {
          fileContent,
          fileName: 'SnakeholeLoungeLogo.png',
          mimeType: 'image/png',
        },
        { subjectType: 'input' }
      )
      cy.get("[data-cy='company-logo-uploaded']").should('have.attr', 'src')
      cy.get("[data-cy='logo-upload-fileName']").contains(
        'SnakeholeLoungeLogo.png'
      )
    })
    // Quill Input
    ...
    cy.get('.ql-editor')
      .last()
      .children()
      .contains('We are the dopest, trillest club around.')
    cy.get('input[name="companyHQ"]')
      .type('Pawnee, IN')
      .should('value', 'Pawnee, IN')
    cy.get("[data-cy='next-step-button']").click()
    ...
    cy.get("[data-cy='role-focus-and-position-type']").contains(
      'Front-end • Full-time'
    )
    cy.get("[data-cy='job-description-title']").contains('Job Description')
    cy.get("[data-cy='job-description']").contains('This is a GREAT gig.')
    cy.get("[data-cy='company-description-title']").contains(
      'About Snakehole Lounge'
    )
    cy.get("[data-cy='company-description']").contains(
      'We are the dopest, trillest club around.'
    )
    cy.get("[data-cy='company-logo']").should('have.attr', 'src')
    cy.get("[data-cy='company-name-sidebar']").contains('Snakehole Lounge')
    cy.get("[data-cy='company-website']").should(
      'have.attr',
      'href',
      'https://parksandrecreation.fandom.com/wiki/The_Snakehole_Lounge'
    )
    cy.get("[data-cy='company-email']").should(
      'have.attr',
      'href',
      'mailto:tom@snakeholelounge.club'
    )
    cy.get("[data-cy='how-to-apply']").should(
      'have.attr',
      'href',
      'https://indeed.com?snakeholeLounge'
    )
    cy.get("[data-cy='how-to-apply-bottom']").should(
      'have.attr',
      'href',
      'https://indeed.com?snakeholeLounge'
    )
